...
template <typename T> struct _type_vec1 {
  ...
  __device__ __forceinline__ void wf_reduce() {
    T t0_sum = 0.0;
    SHFL_DOWN_WF_REDUCE<T, __WF_SIZE__>(t0_sum, data);
    data = t0_sum;
  }
};
...
template <typename T> struct _type_vec3 {
  ...
  __device__ __forceinline__ void wf_reduce() {
    ...
    SHFL_DOWN_WF_REDUCE<T, __WF_SIZE__>(t0_sum, data[0]);
    SHFL_DOWN_WF_REDUCE<T, __WF_SIZE__>(t1_sum, data[1]);
    SHFL_DOWN_WF_REDUCE<T, __WF_SIZE__>(t2_sum, data[2]);
    data[0] = t0_sum;
    data[1] = t1_sum;
    data[2] = t2_sum;
  }

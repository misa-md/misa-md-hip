...
private:
  /**
   * Launch the kernel to calculate electron density rho if the memory layout is Array of Struct mode.
   ...
   */
  void launchKernelMemLayoutAoS(hipStream_t &stream, type_rho_buffer_aos_desc d_p, const _type_atom_count atom_num_calc,
                                const DoubleBuffer::tp_block_item_idx data_start_index,
                                const DoubleBuffer::tp_block_item_idx data_end_index);

  /**
   * Launch the kernel to calculate electron density rho if the memory layout is Struct of Array mode.
   * @param stream Hip Stream
   * @param d_p double buffer descriptor
   * @param atom_num_calc the number of atoms to be calculated in current data block.
   * @param data_start_index block item start index.
   * @param data_end_index block item ending index.
   */
  void launchKernelMemLayoutSoA(hipStream_t &stream, type_rho_buffer_soa_desc d_p, const _type_atom_count atom_num_calc,
                                const DoubleBuffer::tp_block_item_idx data_start_index,
                                const DoubleBuffer::tp_block_item_idx data_end_index);

...
DoubleBuffer::DoubleBuffer(hipStream_t &_stream1, hipStream_t &_stream2, const tp_data_block_id _blocks,
                           const tp_block_item_idx data_len)
...
void DoubleBuffer::getCurrentDataRange(const tp_data_block_id block_i, tp_block_item_idx &index_start,
                                       tp_block_item_idx &index_end) {
  const tp_block_item_idx n_per_block = data_len / blocks;
  index_start = n_per_block * block_i;
  index_end = std::min(index_start + n_per_block, data_len);
}

void DoubleBuffer::fillBufferWrapper(const tp_data_block_id block_id) {
  if (block_id < 0 || block_id >= blocks) {
    return;
  }
  const bool is_left_buffer = (block_id % 2 == 0);
  tp_block_item_idx data_start_index = 0, data_end_index = 0;
...
void DoubleBuffer::fetchBufferWrapper(const tp_data_block_id block_id) {
  if (block_id < 0 || block_id >= blocks) {
    return;
  }
  const bool is_left_buffer = (block_id % 2 == 0);
...
void DoubleBuffer::waitCalc(const tp_data_block_id block_id) {
  if (block_id < 0 || block_id >= blocks) {
    return;
  }
  if (block_id % 2 == 0) {
...
void DoubleBuffer::waitComm(const tp_data_block_id block_id) {
  if (block_id < 0 || block_id >= blocks) {
    return;
  }
  if (block_id % 2 == 0) {
